# AGENTS.md — survivors-maker プロジェクト情報

> このファイルはAIエージェントおよびコラボレーター向けのプロジェクト情報です。
> コードを触る前に必ずここを参照してください。

---

## 🎮 プロジェクト概要

**survivors-maker** は Unity 製の「ヴァンパイアサバイバーズ風ゲームを誰でも作れる」ツクール系エディターです。

- RPGツクールのように **ドット絵を描いて、キャラを動かして、マップを作れる** ことが最終目標
- ユーザーがエディター上で敵・Wave・武器・マップ・パッシブなどを自由に設定できる
- 設定データは **JSON形式** で保存・読み込み
- エディターUIは **UI Toolkit（UIElements）** で統一
- ゲーム内オーバーレイUI（HUD・レベルアップ等）は **OnGUI（IMGUI）** を使用

---

## 🛠️ 技術スタック

| 項目 | 内容 |
|---|---|
| エンジン | Unity（2D） |
| 言語 | C# |
| UIシステム（エディター） | **UI Toolkit（UIElements）**  ← 一般ユーザー向けに使いやすいUIが作れるため |
| UIシステム（ゲーム内HUD） | OnGUI（IMGUI）― GameHUD / LevelUpUI など実行時オーバーレイ限定で許容 |
| 保存形式 | JSON（DataManager経由） |
| 入力 | Unity Input System |
| 物理 | Physics2D |
| レイヤー構成 | Player=8 / Enemy=9 / Projectile=10 / Wall=11 |

---

## 📁 ディレクトリ構成

```
Assets/
├── Scripts/
│   ├── Play/          # ゲームプレイ中のスクリプト
│   ├── Editor/        # エディターパネル（UIElements使用）
│   ├── Data/          # データクラス（WeaponData, EnemyData等）
│   └── Events/        # EventBus用イベント定義
├── Sprites/           # スプライト・スプライトシート置き場
└── GeneratedPrefabs/  # SceneSetupEditorが自動生成するPrefab
```

---

## 📐 コード規約

| 規則 | 内容 |
|---|---|
| フィールド命名 | `_camelCase`（プライベートフィールド） |
| プロパティ | `PascalCase` |
| イベント購読 | `OnEnable` / `OnDisable` で Subscribe / Unsubscribe をペアで行う |
| null安全 | `?.` / `??` を積極的に使用 |
| レイヤー参照 | 番号直書きOK（Player=8, Enemy=9, Projectile=10, Wall=11） |

---

## 🖼️ UIシステム使い分け指針

| 用途 | 使用システム | 理由 |
|---|---|---|
| エディターパネル全般 | **UI Toolkit（UIElements）** | Flexboxレイアウト・ScrollView・スタイリングが一般ユーザー向けに最適 |
| ゲーム内HUD（HP・EXP・タイマー等） | **OnGUI（IMGUI）** | ゲームオーバーレイとしてシンプルに機能するため許容 |
| レベルアップ武器選択UI | **OnGUI（IMGUI）** | 同上 |
| 新規エディターパネル追加時 | **UI Toolkit（UIElements）** | 必ずこちらを使うこと |

> ⚠️ **uGUI（Canvas / TextMeshPro）は使用禁止**

---

## ✅ 実装済み機能

### ゲームプレイ
- [x] プレイヤー移動（WASD / ゲームパッド対応）
- [x] 被弾後の無敵時間（`_damageCooldown` = 0.8秒、インスペクタ調整可）
- [x] 経験値・レベルアップシステム
- [x] 武器システム（近接 / 遠距離 / 範囲）
- [x] 武器一覧：はじめの剣 / 弓 / 爆発 / 短剣 / ハンマー / 光波
- [x] パッシブシステム（HP強化 / 回復 / 速度UP / タフネス / EXP UP）
- [x] レベルアップ時に武器・パッシブ混合3択UI
- [x] 攻撃範囲インジケーター（WeaponRangeIndicator）― 攻撃発動時に半透明円で範囲を表示
- [x] タイトル画面（TitleScreen）― 起動時タイトル表示・スタートボタンでエディターへ遷移
- [x] BGM・SE管理（AudioManager）― 状態別BGM切替・敵撃破/レベルアップ/被弾SEフック・インスペクターでクリップ設定

### 敵
- [x] スライム（近接・追跡型）
- [x] アーチャー（遠距離・射程3f・弾速5f）
- [x] Waveシステム（時間ベース、30秒間隔、難易度スケーリング）

### UI
- [x] ゲームHUD（HP / Lv / 時間 / キル数 / EXPバー）
- [x] リザルト画面（GAME OVER / CLEAR・生存時間・キル数・レベル表示）
- [x] 「もう一度プレイ」ボタン

### エディター（UIElements製）
- [x] `SurvivorsMaker/Setup Scene` メニューからワンクリックシーン構築
- [x] マップ自動生成（`MapGenerator`）
- [x] マップエディター（`MapEditor`）― グリッド描画・タイルペイント・レイヤー切替・Undo/Redo
- [x] 敵エディター（`EnemyEditor`）― フォールバックUIあり・デフォルト敵4種シード実装済み
- [x] 武器エディター（`WeaponEditor`）― 武器データの追加・編集・削除
- [x] Waveエディター（`WaveEditor`）― Wave・SpawnGroup設定・Undo/Redo・JSON入力
- [x] アセットマネージャー（`AssetManager` + `AssetManagerPanel`）― 画像/音声/フォントのインポート・置換・削除
- [x] パッシブエディター（PassiveEditor）― パッシブ種類・数値の追加・編集・削除・デフォルト5種シード実装済み
- [x] 攻撃範囲インジケーター（WeaponRangeIndicator）― 攻撃発動時に半透明円で範囲を表示（近接=赤/遠距離=水色/範囲=黄）
- [x] 全保存・全読込（EditorRootPanel ツールバー）― 💾/📂 ボタンで全エディターを一括保存・読み込み
- [x] プリセット機能（PresetEditor）― 設定をプリセット保存・呼び出し。ビルトイン2種（スライムラッシュ・アーチャー地獄）付属
- [x] スプライト設定エディター（SpriteSettingsEditor）― プレイヤー・敵スプライトをAssetManager連携で設定

### アニメーション
- [x] `PlayerAnimator`：歩行 / アイドル / 被弾 / 死亡アニメーション対応
  - スプライトシート仕様：36×24px / フレーム、右向き、左向きは flipX
  - ※スプライト未設定でも動作する（インスペクタから設定）

---

## 🚧 既知の未解決問題

| 問題 | 状況 | 対応予定 |
|---|---|---|
| Wave間の空白時間（敵全滅後の中折れ） | ✅ 修正済み（Waveリストをソート＋スポーン中フラグ追加） | 完了 |
| 攻撃範囲が視覚的にわからない | ✅ 修正済み（AttackRangeEffect で円形エフェクト表示） | 完了 |
| プレイヤースプライト未設定 | PlayerAnimator は実装済み・null安全確認済み | スプライトシートをインスペクタから設定要 |

---

## 🗺️ 実装予定（ロードマップ）

### フェーズ0：エディター基盤 ← **次のフェーズ**
| # | 機能 | 概要 |
|---|---|---|
| 0-1 | **エディターUI基盤** | 左メニュー＋右コンテンツのRPGツクール型レイアウト（UIElements） |
| 0-2 | **ドット絵エディター** | 32×32キャンバス・パレット・ペン/消しゴム・書き出し |
| 0-3 | **スプライトシート管理** | 描いたドット絵をアニメーション行として管理 |

### フェーズ1：エディター各設定パネル
| # | 機能 | 概要 |
|---|---|---|
| 1-1 | マップ設定エディター | サイズ・壁・床の色をGUIで変更 |
| 1-2 | ゲーム基本設定エディター | 制限時間・プレイヤーHP・移動速度 |

### フェーズ2：敵・Wave設定
| # | 機能 | 概要 |
|---|---|---|
| 2-1 | 敵データエディター | 種類追加・各パラメータ設定 |
| 2-2 | Waveエディター | 時間・スポーン数・敵種類設定。Wave間空白問題も同時解消 |
| 2-3 | 敵種類追加 | 速い弱い型・遅い硬い型など |

### フェーズ3：武器・パッシブ設定
| # | 機能 | 概要 |
|---|---|---|
| 3-1 | 武器エディター | 選択肢の種類・性能設定 |
| ✅ 3-2 | パッシブエディター | パッシブ項目と数値設定 <!-- 実装済み --> |
| ✅ 3-3 | 攻撃範囲の可視化 | 範囲攻撃時にエフェクト表示 <!-- 実装済み --> |

### フェーズ4：保存・読み込み
| # | 機能 | 概要 |
|---|---|---|
| ✅ 4-1 | JSON保存・読み込み | 設定内容をJSONで保存 <!-- 実装済み --> |
| ✅ 4-2 | プリセット機能 | テンプレートゲームを用意 <!-- 実装済み --> |

### フェーズ5：仕上げ
| # | 機能 | 概要 |
|---|---|---|
| ✅ 5-1 | タイトル画面 | スタート画面 <!-- 実装済み --> |
| ✅ 5-2 | BGM・SE | 各種サウンド実装 <!-- 実装済み --> |
| ✅ 5-3 | スプライト設定UI | エディター上でキャラ見た目を変更 <!-- 実装済み --> |

---

## 💬 AIへの指示ガイド

このプロジェクトでAIに作業を依頼する際の注意点：

- **新規エディタースクリプトは `Assets/Scripts/Editor/` に配置**
- **新規プレイ系スクリプトは `Assets/Scripts/Play/` に配置**
- **エディターパネルは必ず UI Toolkit（UIElements）で実装**（`[RequireComponent(typeof(UIDocument))]` を付ける）
- **uGUI（Canvas / TextMeshPro）は使用禁止**
- **ゲーム内HUD・オーバーレイに限り OnGUI は許容**（GameHUD・LevelUpUI 等の既存実装を踏襲する場合）
- **EventBusパターン**でシステム間通信（直接参照よりイベント優先）
- **`SceneSetupEditor.cs`** にシーン構築処理をまとめる（新コンポーネントはここに追加）
- **JSON保存は DataManager 経由**で行う
- コードスタイルは `_camelCase` を厳守

---

*最終更新：2026-06-09*
